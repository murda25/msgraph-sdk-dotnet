...
namespace Microsoft.Graph.Users.Item.Activities.Recent {
    ...
    public class RecentRequestBuilder {
        ...
        public RecentRequestBuilder(Dictionary<string, object> pathParameters, IRequestAdapter requestAdapter) {
            ...
            UrlTemplate = "{+baseurl}/users/{user%2Did}/activities/microsoft.graph.recent()";
        ...
        /// </summary>
        public async Task<RecentResponse> GetAsync(Action<IDictionary<string, string>> headers = default, IEnumerable<IRequestOption> options = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            var requestInfo = CreateGetRequestInformation(headers, options);
            return await RequestAdapter.SendAsync<RecentResponse>(requestInfo, RecentResponse.CreateFromDiscriminatorValue, responseHandler, default, cancellationToken);
        }
    }
}
